...
result = result.filter(r => new Date(r.tanggal_tayang) <= today).reverse().slice(0, 7);
    ...
    // Untuk hasil suara (publikasi di beranda)
    if (table === 'tb_suara') {
      const sheetConfig = SS.getSheetByName('tb_voting_config');
      if (sheetConfig) {
        const configData = sheetConfig.getDataRange().getValues();
        if (configData.length > 1) {
          const isResultPublished = configData[1][6] === true || configData[1][6] === "TRUE" || String(configData[1][6]).toLowerCase() === 'true';
          if (isResultPublished) return responseJson(result);
        }
      }
      return responseJson([]); // Return array kosong jika belum di-publish
    }
